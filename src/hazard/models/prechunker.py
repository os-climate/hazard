...
class PrechunkedSource(OpenDataset):
    def __init__(self, working_dir: Path, 
            prechunked_zarr_store: MutableMapping):
...
class Prechunker:
        ...
        for gcm in self.gcms:  
            for scenario in self.scenarios:
                for quantity in self.quantities:
                    logger.info(f"Prechunking gcm={gcm} scenario={scenario} quantity={quantity}")
    ...
    def _prechunk(self, gcm: str, scenario: str, quantity: str, years: Sequence[int]):
        ...
        with futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_key = {executor.submit(self._download, gcm, scenario, quantity, y): y for y in remaining_years}
        ...
        with open(str(local_file) + ".download", 'wb') as data:
            for attempt in range(5):
                try:
                    ...
                    self.s3.download_fileobj(p.parts[0], str(PurePosixPath(*p.parts[1:])), data)
                    break
                except Exception as e:
                    if attempt == 4:
                        raise e
