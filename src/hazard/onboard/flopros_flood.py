...
class FLOPROSFloodStandardOfProtectionSource(OpenDataset):
    ...
    def prepare(self, working_dir: Optional[str] = None):
        if not isinstance(self.fs, LocalFileSystem):
            ...
            download_and_unzip(
                self.zip_url, working_dir, self.archive_name
            )
            ...
            download_and_unzip(
                self.zip_url, self.source_dir, self.archive_name
            )
        logger.info("Reading database into GeoDataFrame")
        path = Path(self.source_dir) / self.archive_name / "Scussolini_etal_Suppl_info"/ "FLOPROS_shp_V1" / "FLOPROS_shp_V1.shp"
    ...
    def open_dataset_year(
        ...
        def get_merged_rp(row, min_max: str, flood_type: str):
            ...
            for layer in layers: # design layer, policy layer, modelled layer
                ...
                layer_rp = row[f"{layer}_{flood_type}"] if layer == "ModL" else row[f"{layer}_{min_max}_{flood_type}"]
        ...
        logger.info(f"Inferring max and min protection levels per region")
        for _, row in self.df.iterrows():
            flood_type = "Riv" if hazard_type == "RiverineInundation" else "Co" # riverine and coastal        
            min, max = get_merged_rp(row, "Min", flood_type), get_merged_rp(row, "Max", flood_type)
        ...
        width, height = int(60 * 360 / resolution_in_arc_mins), int(60 * 180 / resolution_in_arc_mins) 
        crs, transform = global_crs_transform(width, height)
        logger.info(f"Creating empty array")
        da = empty_data_array(width, height, transform, str(crs), index_values = ["min", "max"])
        ...
        for min_max in ["min", "max"]:
            logger.info(f"Creating raster at {(360 * 60) / width} arcmin resolution for {min_max} protection")
    ...
    def _resources(self) -> Dict[str, HazardResource]:
        ...
        return {
            k: HazardResource(
                    ...
                    indicator_model_gcm="",
                    path="inundation/flopros_" + path_component(k) + "/v1/flood_sop/flood_sop",
                    params={},
