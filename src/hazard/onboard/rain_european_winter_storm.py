...
class RAINEuropeanWinterStorm:
        ...
        for scenario in self.scenario_lookup:
            ...
            for year in ([-1] if scenario == "historical" else self.years):
                for ret in self.returns_lookup:
                    ...
                    logger.info(f"Downloading {filename} from url {url.format(filename=filename)}")
                    filename_ret = download_file(url.format(filename=filename), directory=working_dir, filename=filename)
    ...
    def run_historical(self, target: ReadWriteDataArray, gev_params: List[List[Tuple[float, float, float]]]):
        ...
        scenario = "historical"
        year = -1
        logger.info(f"Processing historical scenario")
        dataarrays = self.dataarrays_for_returns(scenario, year)
        logger.info(f"Extrapolating returns")
        extrapolated = self.extrapolate(dataarrays, gev_params)
        combined = dataarrays + extrapolated
        logger.info(f"Interpolating (reprojection)")
        result = self.interpolate(combined)
        logger.info(f"Saving result")
        target.write(self.resource.path.format(scenario="historical", year=1985), result)
    
    def run_scenario(self, target: ReadWriteDataArray, scenario, year, gev_params: List[List[Tuple[float, float, float]]]):
        """
        ...
        logger.info(f"Calculating returns from probability change.")
    ...
    def dataarrays_for_returns(self, scenario: str, year: int):
        ...
        file_type = "return_level" if scenario == "historical" else "probability_change"
        result = [xr.open_dataset(self.source_dir / 
                    f"wind_speed_{file_type}_{r}_{self.scenario_lookup[scenario]}_{self.year_lookup[year]}.nc")[f"sfcWindmax_{r}_return_level"] 
                    for r in ["5yr", "10yr" , "20yr" , "50yr"]]
        return result    
    
    def get_historical(self):      
        result = [xr.open_dataset(self.source_dir /
                                  f"wind_speed_return_level_{r}_historical_1970-2000.nc")[f"sfcWindmax_{r}_return_level"]
                                  for r in ["5yr", "10yr" , "20yr" , "50yr"]]
        return result
        
    def convert_datasets(self, exceed_datasets, gev_params: List[List[Tuple[float, float, float]]]):
        """
    ...
    def interpolate(self, da_returns_list):
        ...
        for i in range(len(returns)):
            ...
            interp_near = NearestNDInterpolator(list(zip(np.reshape(lon_array, lon_array.size), np.reshape(lat_array, lon_array.size))),
                                          np.reshape(windspeed_array, lon_array.size))
            interp = LinearNDInterpolator(list(zip(np.reshape(lon_array, lon_array.size), np.reshape(lat_array, lon_array.size))),
                                          np.reshape(windspeed_array, lon_array.size))
            mask = np.isnan(interp_near(x, y, distance_upper_bound = 1.0))
            z = interp(x, y)
            z[mask] = np.nan

            da = xr.DataArray(data=z, coords = {'lat': y[:,0], 'lon': x[0, :]}, dims = ['lat', 'lon'])            
    ...
    def _hazard_resource(self) -> HazardResource:
        with open(os.path.join(os.path.dirname(__file__), "rain_european_winter_storm.md"), "r") as f:
            description = f.read()
