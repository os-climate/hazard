...
class WISCWinterStormEventSource(OpenDataset):
    ...
    def peak_annual_gust_speed(self):
        ...
        n_years = 5 * 26
        target = data_array(np.zeros((n_years, first.lat.size, first.lon.size)),
                                  transform=transform,
        ...
        for year_set_index, y in enumerate(self.years):
            ...
            for f in file_list:
                ...
                with xr.open_dataset(f) as eds:
                    ...
                    target.data[year_index, :, :] = np.maximum(target.data[year_index, :, :], eds.wind_speed_of_gust.data[:, :])
                i = i + 1
        return target
    ...
    def fit_gumbel(self, annual_max_gust_speed: xr.DataArray):
        ...
        data = np.zeros((len(return_periods), annual_max_gust_speed.latitude.size, annual_max_gust_speed.longitude.size))
        fitted_speeds = xr.DataArray(data=data, coords={"return_period": return_periods, 
                                                        "latitude": annual_max_gust_speed.latitude, "longitude": annual_max_gust_speed.longitude},
...
class WISCEuropeanWinterStorm(IndicatorModel[str]):
    ...
    def _resource(self) -> HazardResource:
        """Create resource."""
        with open(
            os.path.join(os.path.dirname(__file__), "wisc_european_winter_storm.md"), "r"
        ) as f:
            description = f.read()
