...
def onboard_hazards(
    ...
    for hazard in hazards:
        if hazard in hazard_map:
            try:
                model = initialize_class(hazard, hazard_map, source_dir)

                for resource in model.inventory():
                    updt_inv_hazards.append(resource)

                if model.is_prepared() and not force_prepare:
                    model.prepare(
...
def initialize_class(hazard: str, hazard_map: dict, directory: Optional[str] = None):
