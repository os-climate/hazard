...
class TUDELFTCoastalFlood_inventory():
    def __init__(self):
        pass
...
class TUDELFTCoastalFlood():

    def __init__(self, bucket_name, prefix, zarr_storage, temp_dir) -> None:
        """
        Pan-European data sets of coastal flood probability of occurrence
        ...
        DATA DESCRIPTION:
        GIS-compatible files containing data related to the probablity of 
        coastal floods occurring in Europe under present and future climate. 
        Includes gridded (GeoTIFF) datasets of coastal flood extents 
        (in two variants, with or without flood protection) and water depths. 
        ...
        self.tif_filenames = ['Coastal_flood_depth' + roots[i] for i in range(len(roots))]
    ...
    def create_reduced_nc(self):
        ...
        for i in range(len(self.tif_paths)):
            for rt_i in range(len(self.return_periods_str)):
                ...
                data = data.isel(y=slice(0, 40200, 10), x=slice(0, -1, 10))
...
if __name__ == '__main__':
    ...
    temp_dir = 'data'
    download_zip = False
    JRCRiverFlood_ = TUDELFTCoastalFlood(bucket_name, prefix, zarr_storage, temp_dir)
