...
class OscZarr(WriteDataArray):
    ...
    def new_empty_like(self, path: str, da: xr.DataArray) -> xr.DataArray:
        ...
        z = self._zarr_create(path, da.shape, transform, crs.to_string(), overwrite=True, indexes=da.coords["index"].data)
    ...
    def _zarr_create(self, path: str, shape: Union[np.ndarray, Tuple[int, ...]], 
                     transform: Affine, crs: str, overwrite=False, indexes=None, chunks=None):
        """
        ...
        if isinstance(indexes, np.ndarray) and indexes.dtype in ["int16", "int32", "int64"]:
